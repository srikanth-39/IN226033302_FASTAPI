...
#----------FOR ORDERS PLACED BY A CUSTOMER------------------------
@app.get('/orders/search')
def orders_search(
    name: str = Query(..., min_length=3, description="Customer name")
):
    results = []
    ...
    if not results:
        return {"message": f"No orders found for customer: {name}"}
